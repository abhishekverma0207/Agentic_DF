#!/usr/bin/env python3
"""
FEU-Agentic-Forecasting Demand Forecasting - Diagnostic Crew Runner
=====================================================

Runs ONLY the Diagnostic crew.
...
Prerequisites:
    ...
    - Model Training crew must have been run (model_artifacts/ must exist)

Usage:
    python run_diagnostic.py --config config/config.yaml

Outputs:
    - diagnostics_output/group_diagnostics.csv
    - diagnostics_output/segment_diagnostics.csv
    - diagnostics_output/diagnostics_summary.json
    - diagnostics_output/model_verdict.json
    - diagnostics_output/diagnostics_report.md
    - diagnostics_output/diagnostic_feedback_context.json
"""
...
def setup_logging(log_file: str = "run_diagnostic.log"):
...
def check_prerequisites(artifact_base_path: str) -> bool:
    """Check that all preceding crews have been run."""
    ...
    model_output_dir = os.path.join(artifact_base_path, "model_artifacts")
    ...
    required_files = [
        ...
        # Feature Engineering outputs — format-agnostic markers (the
        # actual existence check below tolerates parquet OR csv for
        # paths whose basename doesn't carry an extension).
        os.path.join(feature_output_dir, "train_features"),
        os.path.join(feature_output_dir, "val_features"),
        # Model Training outputs
        os.path.join(model_output_dir, "final_model_specs.json"),
    ]

    def _exists_either(p: str) -> bool:
        # Bare basename (no extension): parquet OR csv counts.
    ...
    if missing:
        ...
        logging.error("  4. python run_training.py --config config/config.yaml")
        return False

    # Check for validation results
    # Look for validation predictions first (created by training crew)
    val_predictions_path = os.path.join(model_output_dir, "validation_predictions.csv")
    inference_forecast_path = os.path.join(model_output_dir, "inference_forecast.csv")
    if os.path.exists(val_predictions_path):
        logging.info("Validation predictions found - will use for model performance diagnostics")
    elif os.path.exists(inference_forecast_path):
        logging.info("Inference forecast found - will use for model performance diagnostics")
    else:
        logging.warning("Neither validation_predictions.csv nor inference_forecast.csv found")
        logging.warning("Diagnostics will create placeholder outputs")

    logging.info("Prerequisite check passed - All required outputs found")
    return True


# =============================================================================
# Diagnostic Crew Runner
# =============================================================================
...
def run_diagnostic_only(
    ...
    logging.info("\n" + "="*70)
    logging.info("FEU-AGENTIC-FORECASTING DIAGNOSTIC CREW - STANDALONE RUN")
    logging.info("="*70)
    ...
    try:
        ...
        try:
            ...
            cost_tracker.start_crew("Diagnostic Crew")
        ...
        # Run Diagnostic crew
        logging.info("Starting Diagnostic Crew...")
        logging.info("-" * 40)

        result = run_diagnostic_crew(llm=llm, config=cfg, config_yaml_path=config_yaml_path)
        ...
        if cost_tracker:
            try:
                diag_output_dir = os.path.join(cfg.artifact_base_path, "diagnostics_output")
                crew_report = cost_tracker.end_crew("Diagnostic Crew", diag_output_dir)
        ...
        logging.info("\n" + "="*70)
        logging.info("DIAGNOSTIC CREW COMPLETED SUCCESSFULLY")
        logging.info("="*70)
        logging.info(f"Duration: {duration / 60.0:.2f} minutes")
        logging.info(f"\nOutputs:")
        logging.info(f"  - Group diagnostics: {result.group_diagnostics_path}")
        logging.info(f"  - Segment diagnostics: {result.segment_diagnostics_path}")
        logging.info(f"  - Summary JSON: {result.diagnostics_summary_path}")
        logging.info(f"  - Report: {result.diagnostics_report_markdown_path}")
        logging.info("="*70 + "\n")
        ...
        logging.error("\n" + "="*70)
        logging.error("DIAGNOSTIC CREW FAILED")
        logging.error("="*70)
...
def main():
    parser = argparse.ArgumentParser(
        description='Run FEU-Agentic-Forecasting Diagnostic crew only (requires all preceding crews to have run)'
    )
    parser.add_argument(
        '--config',
        type=str,

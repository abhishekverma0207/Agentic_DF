...
# =============================================================================
# DE_FitWeights — STEP 1 of 2: calculate the ensemble weights
# =============================================================================
# For each TRAINING snapshot S:
#   * LEGO present  (DT/, FS/, TF_pristine_allkeys.prediction_xgb) -> SKIP LEGO
#   * LEGO missing  + RUN_LEGO_IF_MISSING=true -> build it via dbutils.notebook.run
#                     on the parametrized DE LEGO entry (LEGO_RUN_DE_PARAM),
#                     DEMANDIQ_RUN_FLAG=false (LEGO + prediction_xgb only, no old DIQ)
...
# weights -> config/de_ensemble_weights.yaml  (+ a Volume copy that survives the cluster).
#
# Calls the SAME validated functions as DIQ_FitWeights (utils.weight_fit) — a turnkey DE
# preset, NOT a reimplementation. Set REPO_PATH (or rely on auto-detect) and "Run All".
#
# STEP 2 = DE_Forecast: takes the weights this writes and produces the forward forecast.
# =============================================================================

# COMMAND ----------

import os, sys
# Repo on the cluster. Auto-detects this notebook's repo root (…/<repo>/notebooks/DE_FitWeights);
# if that fails, hard-set REPO_PATH to your Databricks Repos / workspace clone.
REPO_PATH = ""
try:
    _nb = dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get()
    _root = os.path.dirname(os.path.dirname(_nb))            # …/notebooks/DE_FitWeights -> repo root
...
# ---- DE config — defaults are the confirmed parallel_run locations ----
dbutils.widgets.text("PARENT_DIR",       "/Volumes/pds_feu_931272_dev/eu_dach/transform/DACH/DACH_LEGO/Germany/parallel_run")
dbutils.widgets.text("TRAIN_SNAPSHOTS",  "202601,202606,202611,202616")  # blank -> auto-discover under PARENT_DIR
...
dbutils.widgets.text("ENGINE_THREADS",   "4")            # LightGBM threads / worker  (workers × threads ≈ driver cores; 6×4 is safe on D32)
...
# --- guardrail OFF by default → keep the fitted [w_s,w_c,w_rf] for every (cat×seg); no snap to [0,0,1] ---
dbutils.widgets.dropdown("GUARDRAIL",    "false", ["true", "false"])
dbutils.widgets.text("SEG_SHRINK_K",     "0")            # 0 = raw per-segment coefficients (no shrink toward category; prod default 5000)
dbutils.widgets.text("SEG_MIN_VOLUME",   "0")            # 0 = fit even low-volume segments, don't force [0,0,1] (prod default 1000)
...
os.environ["SEG_SHRINK_K"]      = W("SEG_SHRINK_K")      # 0 → raw per-segment coefficients (no shrink toward category)
os.environ["SEG_MIN_VOLUME"]    = W("SEG_MIN_VOLUME")    # 0 → don't floor low-volume segments to [0,0,1]

# DE conventions (mirror config_de_base_local.yaml market_io)
MARKET, ALLCAT, PANEL, FS_SUB = "de", "All_Cat", "DT", "FS"
BENCH_SUB, BENCH_COL, ACTUALS_COL = "TF_pristine_allkeys", "prediction_xgb", "Actuals"
...
PARENT_DIR_TMPL = f"{PARENT_DIR}/{{snapshot}}/{ALLCAT}"
...
def lego_ready(snap):
    """All LEGO requirements present for this snapshot's All_Cat (DT/, FS/, prediction_xgb)?"""
...
def ensure_lego(snap):
    ...
    if not RUN_LEGO:
        raise RuntimeError(
            f"[{snap}] LEGO missing ({why}) and RUN_LEGO_IF_MISSING=false. Run DE_AllCAT_PARALLEL_RUN "
            f"for {snap} (history_cut_off={add_weeks(snap, -1)}) first, or set RUN_LEGO_IF_MISSING=true.")
    if not LEGO_RUN_NOTEBOOK:
        raise RuntimeError("RUN_LEGO_IF_MISSING=true but LEGO_RUN_NOTEBOOK is blank.")
    print(f"  [{snap}] LEGO missing ({why}) -> running {LEGO_RUN_NOTEBOOK} (DEMANDIQ_RUN_FLAG=false) ...")
    # DEMANDIQ_RUN_FLAG=false -> DT->FE->FS->TF->Combine (DT/FS + prediction_xgb) but SKIP the old
    # per-cat DIQ fan-out; THIS notebook runs the new DIQ next. PARENT_DIR is the exact All_Cat dir.
...
for snap in fit_snaps:
    ...
    info = wf.save_components(
        ...
        parent_dir_template=PARENT_DIR_TMPL,                  # DT join FS panel, exactly like LIVE
        lego_path_template=f"{PARENT_DIR_TMPL}/{BENCH_SUB}",  # prediction_xgb benchmark
...
# Volume copy so the weights survive the (ephemeral) job cluster.
import shutil
...
print(f"STACK deployed for {n_stack}/{len(score)} (category x segment) groups (rest fall back to pure prediction_xgb).")
...
# MAGIC %md
# MAGIC ## Done — weights fitted.  NEXT: STEP 2 = `DE_Forecast`
# MAGIC
# MAGIC `config/de_ensemble_weights.yaml` is written (and copied to the Volume,
# MAGIC printed above as `VOLUME_WEIGHTS`). Run **`DE_Forecast`** next to apply these
# MAGIC weights and produce the forward forecast — point its `WEIGHTS_PATH` at either
# MAGIC the committed `config/de_ensemble_weights.yaml` or the Volume copy above.